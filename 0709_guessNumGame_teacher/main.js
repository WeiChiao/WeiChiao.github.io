...
//先隱藏遊戲
document.querySelector("#the_game").hidden=true;
// document.querySelector("#game_intro").hidden=true;

window.addEventListener('scroll', function() {
  const goPlayElement = document.querySelector('.go_play');
  const revealPoint = 1500; // 根據需要調整滾動位置點
  
  // 當滾動到指定位置時顯示 .go_play 元素
  if (window.scrollY >= revealPoint) {
      goPlayElement.classList.add('show');
  } else {
      goPlayElement.classList.remove('show');
  }
});



document.querySelector(".go_play").onclick=()=>{



  document.querySelector("#game_intro").hidden=true;
  document.querySelector("#the_game").hidden=false;
  document.querySelector("#the_game").style.backgroundColor = "red";
